...
/* ── StatCard ── */
function StatCard({ label, value, icon, color = 'blue', sub, loading }) {
...
function EmptyRow({ cols, text }) {
  return (
    <tr>
      <td colSpan={cols} style={{ padding: '32px', textAlign: 'center', color: '#334155', fontSize: '13px' }}>
        {text}
      </td>
    </tr>
  );
...
export default function Dashboard() {
  ...
  const [actionId,   setActionId]   = useState(null);   // leave being processed
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      const [dashRes, empRes, leaveRes] = await Promise.all([
        api.get('/dashboard/'),                                       // DashboardView
        api.get('/employees/?ordering=-created_at&page_size=5'),      // EmployeeViewSet list
        api.get('/leaves/?status=pending'),                           // LeaveRecordViewSet
      ]);
  ...
  /* ── LeaveRecordViewSet.approve / .reject ── */
  const handleLeave = async (leaveId, action) => {
    try {
      ...
      setStats((prev) =>
        prev
          ? { ...prev, pending_leaves: Math.max(0, (prev.pending_leaves || 1) - 1) }
          : prev
      );
    } catch {
      alert(`Could not ${action} leave. Please try again.`);
    } finally {
      setActionId(null);
    }
  };
  ...
  return (
    <div style={css.root}>
      ...
      {/* ══════════════ STAT CARDS — DashboardView ══════════════ */}
      <div style={css.statsGrid}>
        <StatCard label="Total Employees"  value={stats?.total_employees}     icon="👥" color="blue"   loading={loading} />
        <StatCard label="Active"           value={stats?.active_employees}     icon="✅" color="green"  loading={loading} />
        <StatCard label="On Leave"         value={stats?.on_leave}             icon="🏖" color="amber"  loading={loading} />
        <StatCard label="Departments"      value={stats?.total_departments}     icon="🏢" color="purple" loading={loading} />
        <StatCard label="New This Month"   value={stats?.new_hires_this_month} icon="🆕" color="teal"   loading={loading} />
        <StatCard
      ...
      {/* ══════════════ ROW 2: Employees + Dept Breakdown ══════════════ */}
      <div style={css.row2}>

        {/* EmployeeViewSet — last 5 */}
        <Card>
          <CardHeader
            title="Recent Employees"
            sub="Latest 5 records · EmployeeViewSet"
            right={
              <a href="/employees" style={css.linkBtn}>View All →</a>
            }
          />
            ...
            <table style={css.table}>
              ...
              <tbody>
                {loading
                  ? <LoadingRow cols={5} />
                  : employees.length === 0
                    ? <EmptyRow cols={5} text="No employees found." />
        ...
        {/* DashboardView — department_breakdown */}
        <Card>
      ...
      <div style={css.row3}>
        ...
        {/* LeaveRecordViewSet — pending · approve / reject actions */}
        <Card>
          <CardHeader
            title="Pending Leave Requests"
            sub={`${pendingLeaves.length} awaiting approval · PATCH /leaves/{id}/approve/`}
          />
          <div style={{ overflowX: 'auto' }}>
            <table style={css.table}>
              <thead>
              ...
              <tbody>
                {loading
                  ? <LoadingRow cols={6} />
                  : leaves.length === 0
                    ? <EmptyRow cols={6} text="🎉 No pending leave requests!" />
                    : leaves.map((leave) => (
                        <tr
                          key={leave.id}
        ...
        {/* DashboardView — employment_type_breakdown */}
        <Card>
              ...
              <>
                ...
                {empTypeEntries.map(([type, count], i) => (
                  <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '11px' }}>
                    ...
                    <span style={{ fontSize: '12px', fontWeight: '700', color: '#e2e8f0', ...mono }}>
                      {count}
                    </span>
